...
if(isset($_POST['bsimpan'])) {
    ...
    if($_GET['hal'] == "edit") {
        ...
        $stmt = mysqli_prepare($koneksi, "UPDATE promo SET
                                            picture = ?,
                                            promo = ?,
                                            descriptions = ?,
                                            validdate = ?
        ...
        // Binding parameter
        mysqli_stmt_bind_param($stmt, 'ssssi', $_POST['picture'], $_POST['promo'], $_POST['descriptions'], $_POST['validdate'], $_GET['id']);
        ...
        if(mysqli_stmt_affected_rows($stmt) > 0) {
            echo "<script>
                    ...
                    document.location='promodata.php';
                  </script>";
        } else {
            echo "<script>
        ...
        // Persiapan statement SQL
        $stmt = mysqli_prepare($koneksi, "INSERT INTO promo (picture, promo, descriptions, validdate)
                                            VALUES (?, ?, ?, ?)");
        
        // Binding parameter
        mysqli_stmt_bind_param($stmt, 'ssss', $_POST['picture'], $_POST['promo'], $_POST['descriptions'], $_POST['validdate']);
...
if(isset($_POST['bsimpan'])) {
    ...
    $folder = "D:\xampp\htdocs\Moon Dental\image\Promo"; // Lokasi folder untuk menyimpan gambar
...
if(isset($_GET['hal'])) {
    ...
    if($_GET['hal'] == "edit") {
        ...
        $stmt = mysqli_prepare($koneksi, "SELECT * FROM promo WHERE id = ?");
        ...
        $stmt = mysqli_prepare($koneksi, "DELETE FROM promo WHERE id = ?");
        mysqli_stmt_bind_param($stmt, 'i', $_GET['id']);
        mysqli_stmt_execute($stmt);
...
<div class="sidebar d-flex flex-column flex-shrink-0 p-3 bg-body-tertiary" style="width: 200px; height: 100vh;">
  ...
  <div class="container">
	...
	<div class="mb-5">
	<span class="subtitle fw-semibold"><center>Moon Dental Clinic Promo Data</center></span>
	</div>
    ...
    <div class="col-8">
        ...
        <div class="form-group row">
          <label class="col-4 col-form-label" for="promo">Promo</label> 
          <div class="col-8">
            <input id="promo" name="promo" placeholder="Promo" type="text" class="form-control" value="<?=isset($vpromo) ? $vpromo : ''?>" required="required">
          </div>
        </div>
        <div class="form-group row">
          <label for="descriptions" class="col-4 col-form-label">Descriptions</label> 
          <div class="col-8">
            <input id="descriptions" name="descriptions" placeholder="Descriptions" type="text" class="form-control" value="<?=isset($vdescriptions) ? $vdescriptions : ''?>" required="required">
          </div>
        </div>
        <div class="form-group row">
          <label for="validdate" class="col-4 col-form-label">Valid Date</label> 
          <div class="col-8">
            <input id="validdate" name="validdate" placeholder="Valid Date" type="text" class="form-control" value="<?=isset($vvaliddate) ? $vvaliddate : ''?>" required="required">
          </div>
        </div>
	...
	<div class="table-container">
		<span class="subtitle fw-semibold"><center>Promo Table</center></span>
		  	<table class="table  table-striped mt-5">
				  ...
				  <th class="col-1">ID</th>
                        ...
                        <th class="col-1">Valid Date</th>
                        <th class="col-2">Action</th>

		  		</tr>
		  		<?php
				$no = 1;
				$tampil = mysqli_query($koneksi, "SELECT * from promo order by id desc");
            ...
            <td><?=$data['validdate']?></td>
		  			<td>
		  			<a href="promodata.php?hal=edit&id=<?=$data['id']?>"><i class="fa-solid fa-pen-to-square"></i></a>
            &nbsp
            &nbsp
		  			<a href="promodata.php?hal=hapus&id=<?=$data['id']?>" onclick="return confirm('Are you sure want to delete this data?')"><i class="fa-solid fa-trash" style="color: red;" ></i></a>
				</td>
		  		</tr>
