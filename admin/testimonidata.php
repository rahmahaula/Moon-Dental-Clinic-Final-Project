...
if(isset($_POST['bsimpan'])) {
    ...
    if($_GET['hal'] == "edit") {
        ...
        $stmt = mysqli_prepare($koneksi, "UPDATE testimoni SET
                                            testimoni = ?,
                                            sourcename = ?,
                                            WHERE id = ?");
        
        // Binding parameter
        mysqli_stmt_bind_param($stmt, 'ssi', $_POST['testimoni'], $_POST['sourcename'], $_GET['id']);
        ...
        if(mysqli_stmt_affected_rows($stmt) > 0) {
            echo "<script>
                    ...
                    document.location='testimonidata.php';
                  </script>";
        } else {
            echo "<script>
        ...
        // Persiapan statement SQL
        $stmt = mysqli_prepare($koneksi, "INSERT INTO testimoni (testimoni, sourcename)
                                            VALUES (?, ?)");
        
        // Binding parameter
        mysqli_stmt_bind_param($stmt, 'ss', $_POST['testimoni'], $_POST['sourcename']);
...
if(isset($_GET['hal'])) {
    ...
    if($_GET['hal'] == "edit") {
        ...
        $stmt = mysqli_prepare($koneksi, "SELECT * FROM testimoni WHERE id = ?");
        ...
        if($data) {
            ...
            $vtestimoni = $data['testimoni'];
            $vsourcename = $data['sourcename'];
        }
        ...
        $stmt = mysqli_prepare($koneksi, "DELETE FROM testimoni WHERE id = ?");
        mysqli_stmt_bind_param($stmt, 'i', $_GET['id']);
        mysqli_stmt_execute($stmt);
...
<head>
    ...
    <title>Admin | Testimoni Data</title>
...
<div class="sidebar d-flex flex-column flex-shrink-0 p-3 bg-body-tertiary" style="width: 200px; height: 100vh;">
	...
	<li>
	  ...
	  <a style="background-color: red;" class="btn btn-reserve fw-medium w-100" href="../view/index.php" role="button" onclick="return confirm('Are you sure want to log out?')" >LOGOUT</a>
      </li>
    </ul>
    <hr>
    <!--<<div class="dropdown">
  ...
  <div class="container">
	...
	<div class="mb-5">
	<span class="subtitle fw-semibold"><center>Moon Dental Clinic Testimoni Data</center></span>
	</div>
	<div class="form-group row">
    <label class="col-4 col-form-label" for="testimoni">Testimoni</label> 
    <div class="col-8">

        <input type="text" class="form-control" id="testimoni" name="testimoni" Placeholder="Testimoni"  value="<?=isset($vtestimoni) ? $vtestimoni : ''?>" >

    </div>
    </div>
    <div class="form-group row">
    <label class="col-4 col-form-label" for="sourcename">Source</label> 
    <div class="col-8">

        <input type="text" class="form-control" id="sourcename" name="sourcename" Placeholder="Source" value="<?=isset($vsourcename) ? $vsourcename : ''?>" >
	...
	<div class="table-container">
		<span class="subtitle fw-semibold"><center>Testimoni Table</center></span>
		  	<table class="table  table-striped mt-5">
		  		<tr>
				  <th class="col-1">ID</th>
                        <th class="col-2">Testimoni</th>
		  		...
		  		<?php
				$no = 1;
				$tampil = mysqli_query($koneksi, "SELECT * from testimoni order by id desc");
				while($data = mysqli_fetch_array($tampil)) :
				?>

		  		<tr>
		  			<td><?=$data['id']?></td>
		  			<td style="width: 75%;" ><?=$data['testimoni']?></td>
                      <td><?=$data['sourcename']?></td>
		  			<td>
		  			<a href="testimonidata.php?hal=edit&id=<?=$data['id']?>"><i class="fa-solid fa-pen-to-square"></i></a>
                    &nbsp
                    &nbsp
		  			<a href="testimonidata.php?hal=hapus&id=<?=$data['id']?>" onclick="return confirm('Are you sure want to delete this data?')"><i class="fa-solid fa-trash" style="color: red;" ></i></a>
				</td>
		  		</tr>
